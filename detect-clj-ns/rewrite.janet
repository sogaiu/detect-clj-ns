...
(comment
  ...
  # XXX: this parses as @[[:number "1"]] because rest of input not done
    ...
    (let [src
          (slurp
            (string (os/getenv "HOME")
              "/src/clojars-cljish/"
              "org/clojars/"
              "kingfranz/seesaw/2.0.6/seesaw-2.0.6.jar/seesaw/core.clj"))
          reconstructed (code (ast src))]
      (if (not= (string src)
            reconstructed)
        (do
          (print "round-trip failed")
          (spit "/tmp/mismatched.clj" reconstructed)
          false)
        true))
